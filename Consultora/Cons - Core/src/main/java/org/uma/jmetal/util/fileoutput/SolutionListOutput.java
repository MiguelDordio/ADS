...
public class SolutionListOutput {
  ...
  public void printObjectivesToFile(FileOutputContext context, List<? extends Solution<?>> solutionList) {
    ...
    try {
      ...
      // fill output object with solutions
      bufferedWriter.close();
    } catch (IOException e) {
...
public AlgorithmResult getAlgorithmResult(List<? extends Solution<?>> solutionList) {
    AlgorithmResult algorithmResult = new AlgorithmResult();

    List<List<Integer>> solutionsResults = new ArrayList<>();
    List<List<Integer>> evaluationsResults = new ArrayList<>();

    int numberOfVariables = solutionList.get(0).getNumberOfVariables();
    int numberOfObjectives = solutionList.get(0).getNumberOfObjectives();

    if (solutionList.size() > 0) {
      //Get variables and Objectives

      for (int i = 0; i < solutionList.size(); i++) {
        List<Integer> solRes = new ArrayList<>();
        List<Integer> evalRes = new ArrayList<>();

        for (int j = 0; j < numberOfVariables; j++) {
          solRes.add((Integer) solutionList.get(i).getVariableValue(j));
        }
        solutionsResults.add(solRes);
        for (int j = 0; j < numberOfObjectives; j++) {
          evalRes.add((int) solutionList.get(i).getObjective(j));
        }
        evaluationsResults.add(evalRes);
      }

      algorithmResult.setSolutions(solutionsResults);
      algorithmResult.setEvaluations(evaluationsResults);
    }
